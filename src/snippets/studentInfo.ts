...
async function fetchDataFromFirestore() {
  const evalCol = new FirestoreCollection<EvaluateCollectionType>("evaluate").setDefaultMutator(Mutators.SpecificKeyFieldKVMutator((doc) => doc.get("student_id")))
  const users = new FirestoreCollection<UserDataCollectionType>("data").setDefaultMutator(Mutators.SpecificKeyFieldKVMutator((doc) => doc.get("student_id")))
  const [evalData, userData] = await Promise.all([evalCol.readFromCache(true), users.readFromCache(true)])
  return { evalData, userData }
}

function getStudentInfo(userData: DMap<string, ReferableMapEntity<IUserData>>, evalRecords: DMap<string, ReferableMapEntity<EvaluateType>>, key: string, searchOption: SearchOption): StudentInfo | undefined {
...
function getStudentMatchName(userData: DMap<string, ReferableMapEntity<IUserData>>, key: string): string[][] {
  ...
  return students.map(student => [`${student.get("student_id")}`, `${student.get("firstname")} ${student.get("lastname")}`])
}
